...
/* This file contains the implementation of the FMI functions
   Please check the fmi2Functions.h file for more details
*/

#if !defined FMI2_FUNCTION_PREFIX && !defined NO_FUNCTION_PREFIX
#define FMI2_FUNCTION_PREFIX ControllerTilt_
#endif
 ...
 * memory
 */
const char* ControllerTilt_URIToNativePath(xx_ModelInstance* model_instance, const char* uri)
{
								...
								const fmi2Real value[])
{
	...
	for (i = 0; i < nvr; i++)
	{
		...
		if ((vr[i] >= xx_parameter_index_start) && (vr[i] <= xx_parameter_index_end))
		{
								...
								fmi2Boolean loggingOn)
{
	...
	model_instance->resourceLocation = ControllerTilt_URIToNativePath(model_instance, fmuResourceLocation);
					...
					fmi2Boolean noSetFMUStatePriorToCurrentPoint)
{
	...
	while (model_instance->time < (currentCommunicationPoint + communicationStepSize))
	{
		...
		if ( model_instance->m_use_finish_time && (model_instance->time >= model_instance->finish_time) )
		{
			...
			{
				model_instance->fmiCallbackFunctions->logger(NULL, "ControllerTilt", fmi2Error, "error",
