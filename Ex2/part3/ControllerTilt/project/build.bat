...
set "FMU=%ROOTPATH%\ControllerTilt.fmu"
set DLL=ControllerTilt.dll
set "ZIPTOOL=C:\Program Files (x86)\20-sim 4.7\bin\7z.exe"
rem Tools found in the 20-sim bin directory (starting from 20-sim 4.6)
set "XSLTTOOL=C:\Program Files (x86)\20-sim 4.7\bin\msxsl.exe"
set "GUIDTOOL=C:\Program Files (x86)\20-sim 4.7\bin\GenerateGuid.exe"
...
set RESOURCES_SCRIPT="C:\Program Files (x86)\20-sim 4.7\Ccode\TERRA20FMU\bin\include_resources.py"
...
ECHO ------------------------------------------------------------
ECHO 20-sim standalone co-simulation FMU export for 'ControllerTilt'
ECHO ------------------------------------------------------------
...
REM for TERRA
ECHO Generating the ControllerTilt.xml
"%XSLTTOOL%"  "%ROOTPATH%\src\ModelConfiguration.xml" "%ROOTPATH%\template\mcf2modelDescription.xsl" -o "%FMU_DIR%\ControllerTilt.xml" SOURCEDIRECTORY="%ROOTPATH%\src"
...
IF EXIST %PYTHON% (
	...
	ECHO Please re-install 20-sim 4.7 with Python support enabled.
)
...
:COMPILE_FMU
  ...
  msbuild.exe "%PROJ_DIR%\ControllerTilt.sln" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\ControllerTilt.sln" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:COMPILE_NO_CLEAN_FMU
  ECHO Compiling 20-sim FMU for submodel "ControllerTilt"
  msbuild.exe "%PROJ_DIR%\ControllerTilt.vcxproj" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\ControllerTilt.vcxproj" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:FMU_READY
  cd ..
  ECHO Your ControllerTilt FMU is ready and can be found in:
  ECHO   %CD%
