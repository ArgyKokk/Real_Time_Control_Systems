...
set "FMU=%ROOTPATH%\ControllerPan.fmu"
set DLL=ControllerPan.dll
...
ECHO ------------------------------------------------------------
ECHO 20-sim standalone co-simulation FMU export for 'ControllerPan'
ECHO ------------------------------------------------------------
...
REM for TERRA
ECHO Generating the ControllerPan.xml
"%XSLTTOOL%"  "%ROOTPATH%\src\ModelConfiguration.xml" "%ROOTPATH%\template\mcf2modelDescription.xsl" -o "%FMU_DIR%\ControllerPan.xml" SOURCEDIRECTORY="%ROOTPATH%\src"
...
:COMPILE_FMU
  ...
  msbuild.exe "%PROJ_DIR%\ControllerPan.sln" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\ControllerPan.sln" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:COMPILE_NO_CLEAN_FMU
  ECHO Compiling 20-sim FMU for submodel "ControllerPan"
  msbuild.exe "%PROJ_DIR%\ControllerPan.vcxproj" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\ControllerPan.vcxproj" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:FMU_READY
  cd ..
  ECHO Your ControllerPan FMU is ready and can be found in:
  ECHO   %CD%
