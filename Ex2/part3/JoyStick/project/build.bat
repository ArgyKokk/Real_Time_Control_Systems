...
set "FMU=%ROOTPATH%\JoyStick.fmu"
set DLL=JoyStick.dll
...
ECHO ------------------------------------------------------------
ECHO 20-sim standalone co-simulation FMU export for 'JoyStick'
ECHO ------------------------------------------------------------
...
REM for TERRA
ECHO Generating the JoyStick.xml
"%XSLTTOOL%"  "%ROOTPATH%\src\ModelConfiguration.xml" "%ROOTPATH%\template\mcf2modelDescription.xsl" -o "%FMU_DIR%\JoyStick.xml" SOURCEDIRECTORY="%ROOTPATH%\src"
...
:COMPILE_FMU
  ...
  msbuild.exe "%PROJ_DIR%\JoyStick.sln" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\JoyStick.sln" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:COMPILE_NO_CLEAN_FMU
  ECHO Compiling 20-sim FMU for submodel "JoyStick"
  msbuild.exe "%PROJ_DIR%\JoyStick.vcxproj" /p:Configuration=%buildconfig%;Platform=win32 /t:Build /verbosity:minimal
  IF %BUILD_X64%==1 (
    msbuild.exe "%PROJ_DIR%\JoyStick.vcxproj" /p:Configuration=%buildconfig%;Platform=x64 /t:Build /verbosity:minimal
  )
...
:FMU_READY
  cd ..
  ECHO Your JoyStick FMU is ready and can be found in:
  ECHO   %CD%
