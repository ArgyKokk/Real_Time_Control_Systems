/**
 * Header file for the LinearSystem model
 * Generated by the TERRA CSPm2LUNA generator version 1.1.3
 *
 * protected region document description on begin
 *
 * protected region document description end
 */

#pragma once

#include "csp/CSP.h"

// Models
#include "XXLinearSystemModel.h"

// protected region additional headers on begin
// Each additional header should get a corresponding dependency in the Makefile
// protected region additional headers end

using namespace LUNA::CSP;

namespace LinearSystem { 

class LinearSystem : public Sequential
{

public:
  // Define constructor and destructor
  LinearSystem(ChannelIn<double> *time_to_display, ChannelOut<double> *u, ChannelIn<double> *y, ChannelIn<double> *y_out);
  virtual ~LinearSystem();

private:

  // Class variables
  double time;
  double v_u;
  double v_y;

  // Model objects
  XXLinearSystemModel::XXLinearSystemModel *myXXLinearSystemModel;
  Writer<double> *myY_to_display;
  Reader<double> *myr_u;
  Writer<double> *myw_y;
  Writer<double> *mywriter_time;

  // Model groups
  PriParallel *myPRI_PARALLEL;



  // protected region additional class members or functions on begin
  // protected region additional class members or functions end
};

// Close namespace(s)
} 
