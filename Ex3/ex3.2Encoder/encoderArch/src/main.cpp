...
#if CONFIG_XENO_VERSION_MAJOR < 3 && CONFIG_XENO_VERSION_MINOR < 7 && CONFIG_XENO_REVISION_LEVEL < 3
...
// protected region Application init on begin
// protected region Application init end  


  // Check if Xenomai version 2.6.2 or lower is used
  #if CONFIG_XENO_VERSION_MAJOR < 3 && CONFIG_XENO_VERSION_MINOR < 7 && CONFIG_XENO_REVISION_LEVEL < 3
  // We need to manually call this method (to properly handle the memory), in Xenomai 2.6.3+ this is done automatically
...
// protected region RTlogger support on begin
  ...
  else
  {
    ...
    else
    {
      if(OSScheduler::Instance()->setSchedulerPolicy(FIFO))
      {
        LOGLOCAL(LOG_ERROR, "Could not set the scheduler policy to FIFO. Do you have root permissions?\n");
        exit(EXIT_FAILURE);
      }
...
// protected region RTlogger support end  
  ...
  LOGLOCAL(LOG_INFO, "Setting up a signal handler, press ctrl+c to stop encoderArch.\n");
  ...
  LOGLOCAL(LOG_INFO, "Constructing the encoderArch structure:\n");
...
// protected region manual tree modifications end  
  ...
  LOGLOCAL(LOG_INFO, "Starting encoderArch - term thread: %p\n", term->getIThread());
  term->activate();
...
// protected region additional code end
  ...
  LOGLOCAL(LOG_INFO, "encoderArch stopped.\n");
